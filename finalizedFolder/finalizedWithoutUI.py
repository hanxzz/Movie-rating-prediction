...
rating_prompt = (
    ...
    f"Answer only with the floating point number You can also use decimal values for example: 1.3, 3,8, etc.\n\nScript: {script_text}\n\nRating:"
)
...
rating_response = requests.post(API_URL, headers=HEADERS, json={"inputs": rating_prompt, "parameters": {"max_new_tokens": 5}})
rating_text = rating_response.json()[0]["generated_text"]

matches = re.findall(r'\d+\.\d+|\d+', rating_text)
rating = matches[-1] if matches else "N/A"

if(script_text == " " or not script_text):
    ...
    improvement_response = requests.post(API_URL, headers=HEADERS, json={"inputs": improvement_prompt, "parameters": {"max_new_tokens": 150}})
    improvements = improvement_response.json()[0]["generated_text"].replace(improvement_prompt, "").strip()
    print("----------------------------------")
